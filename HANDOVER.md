...
├── kids-scripts/                 # 【サイト非公開】ちびっ子向けYouTube台本の置き場
│   └── YYYY-MM-DD-N.md           # 5,000〜10,000字の語り台本（やさしい版記事の元ネタ）
├── .github/workflows/
...
| 項目 | 月額目安 |
|---|---|
| Claude API（Sonnet 4.5、6回/日 × 30日 ※2026-09-19に台本生成を追加して4回→6回） | 約1,500〜3,000円 |
...
## 9-2. やさしい版は「2段生成」（2026-09-19〜）

やさしい版は **台本 → 要約記事** の2段構えで作っている（本人決定 2026-09-19：「10,000字に広げるのは台本収納でいい。で、要約版で記事にする形」）。

| 段 | 何を作る | 長さ | tool | max_tokens | 出力先 |
|---|---|---|---|---|---|
| ① 台本 | ちびっ子向けYouTubeの語り台本 | 5,000〜10,000字 | `write_kids_script`（title / body） | 16000 | `kids-scripts/YYYY-MM-DD-N.md` |
| ② 記事 | ①を要約した「【やさしい版】」記事 | 1,500〜2,000字 | `write_blog_article` | 4000 | `src/content/blog/YYYY-MM-DD-N-kids.md` |

- **`kids-scripts/` はサイトに出ない。** Astroのコレクションは `src/content/blog` だけを読むので、台本は `npm run build` しても `dist/` に出力されない。YouTube台本の源泉としてGitに貯めるだけ。
  - ⚠️ ただし**このリポジトリは公開（public）**なので、台本もGitHub上では誰でも読める。
- **記事の型はA案が標準**：ニュースを一言で → なぜ①（こまる側）＋たとえば2つ（各200〜300字）→ なぜ②（うれしい側）＋たとえば2つ → 数字を身近にしてみると → じゃあ、うちはどうする？
  - 制度もの・賛否が割れる話・うれしい側の例が2つ作れないときは**B案**（なぜ2つ × いいこと／こまること × 各2例を1〜2文で）に切り替えてよい。B案は「2文で言い切れない論点は入れない」が必須ルール。
  - 検討の元資料：`~/Desktop/コード/データ/kakei_news_kids_AB比較_20260919.md`
- **フォールバック**：台本が `max_tokens` で切れたら1回だけリトライ。それでも切れたら台本は保存せず、やさしい版は従来どおりニュースから直接1段で生成する（記事が出ない事故を避けるため）。
- 大人版（detail）の生成は一切変えていない。
